# Development E2E runbook

Run from this directory. Set `EVIDENCE="$(git rev-parse --show-toplevel)/artifacts/e2e/$(date -u +%Y%m%dT%H%M%SZ)"` and `mkdir -p "$EVIDENCE"`. Every `tee` output below is evidence; do not store kubeconfigs, tokens, certificates, or private keys.
...
Expected: all three namespaces have `ksp.io/environment=dev` and `ksp.io/profile=baseline`. These YAML exports are E2E/config evidence, not a disaster-recovery or etcd backup.
